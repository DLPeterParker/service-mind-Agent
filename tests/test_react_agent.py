...
async def test_order_query():
    ...
    _clean()
    agent = _fresh_agent()
    resp = await agent.chat("帮我查一下订单 ORD-20240115-001 的状态")
...
async def test_logistics_query():
    ...
    _clean()
    agent = _fresh_agent()
    resp = await agent.chat("订单 ORD-20240115-001 的物流到哪了？")
...
async def test_product_search():
    ...
    _clean()
    agent = _fresh_agent()
    resp = await agent.chat("你们有什么耳机卖？")
...
async def test_refund():
    ...
    _clean()
    agent = _fresh_agent()
    resp = await agent.chat("我要退掉订单 ORD-20240110-003，质量有问题")
...
async def test_unknown_order():
    ...
    _clean()
    agent = _fresh_agent()
    resp = await agent.chat("帮我查订单 ORD-99999-000")
...
# ---------- 测试 7：会话持久化兼容 ----------
@pytest.mark.asyncio
async def test_persistence_with_tools():
    print("\n[7/7] 会话持久化兼容测试（工具消息也能保存和恢复）")
    _clean()
    agent = _fresh_agent()
    await agent.chat("帮我查一下订单 ORD-20240120-002")
    ...
    resp = await agent2.chat("这个订单什么时候能发货？")
