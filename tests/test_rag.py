...
def test_build_index(backend_name: str):
    ...
    try:
        backend.load()
        if (
            backend.expected_embedding_model() == settings.embedding_model
            and backend.size() > 0
        ):
...
def test_retriever_top_k(backend_name: str):
    ...
    for query, expected_doc in cases:
        ...
        if top.chunk.doc != expected_doc:
            ...
            _ok(
                f"query={query!r} → {top.chunk.doc} / {top.chunk.section}（{top.score:.3f}）"
            )
...
def test_agent_end_to_end(backend_name: str):
    ...
    try:
        ...
        triggered = any(
            ...
            and any(
                tc["function"]["name"] == "search_knowledge" for tc in m["tool_calls"]
            )
