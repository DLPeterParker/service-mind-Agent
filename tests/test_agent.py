...
async def test_structured_output():
    ...
    for user_input, expected_intents in TEST_CASES:
        ...
        print(
            f"  意图: {response.intent.value} (期望: {[i.value for i in expected_intents]})"
        )
...
async def test_multi_turn():
    ...
    # 清理上次运行的残留 session 文件
    session_file = Path("app/sessions/test_agent_multi_turn_session.json")
    session_file.unlink(missing_ok=True)

    agent = EcomAgent(user_id="test_agent_multi_turn")
    agent.history_threshold = 999  # 避免历史 session 触发压缩干扰测试

    # 第一轮：提出问题
    r1 = await agent.chat("我想退掉上周买的那双运动鞋")
    print("\n[轮次1] 输入: 我想退掉上周买的那双运动鞋")
    print(f"  回复: {r1.reply[:80]}...")

    # 第二轮：基于上下文追问（不再提"运动鞋"，看模型是否记住）
    r2 = await agent.chat("穿了一次，鞋底就开胶了")
    print("\n[轮次2] 输入: 穿了一次，鞋底就开胶了")
    print(f"  回复: {r2.reply[:80]}...")

    # 验证对话历史长度（每轮产生 1 user + 2 assistant 消息）
    expected_len = 6  # 2 user + 4 assistant（含结构化输出）
    actual_len = len(agent.raw_messages)
...
async def test_reset():
    ...
    agent = EcomAgent(user_id="test_agent_reset")
    await agent.chat("你好")

    assert len(agent.raw_messages) >= 3  # user + assistant text + assistant structured

    agent.reset()
    assert len(agent.raw_messages) == 0  # reset 清空全部
