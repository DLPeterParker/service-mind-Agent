...
TEST_USER_ID = "test_conv_mgmt"
TEST_SESSION_DIR = str(ROOT / "app" / "sessions")
TEST_SESSION_FILE = str(ROOT / "app" / "sessions" / f"{TEST_USER_ID}_session.json")
TEST_SESSION_LEGACY = str(ROOT / "app" / "sessions" / "test_session.json")


def _fresh_agent(threshold: int = 20, keep: int = 6) -> EcomAgent:
    """构造测试用 Agent：使用独立 user_id 隔离会话。"""
    agent = EcomAgent(
        user_id=TEST_USER_ID,
        session_path=TEST_SESSION_LEGACY,
    )
...
def _clean():
    Path(TEST_SESSION_FILE).unlink(missing_ok=True)
    Path(TEST_SESSION_LEGACY).unlink(missing_ok=True)
...
# ---------- 测试 1：记忆压缩测试 ----------
@pytest.mark.asyncio
async def test_summary_triggered():
    ...
    # 1 表示序号从 1 开始计数。每次循环，i 拿到序号，p 拿到具体的句子。
    for i, p in enumerate(prompts, 1):  # 模拟多轮对话
    ...
    if "A123456" not in agent.summary:
        print("  ⚠️  summary 中未检索到订单号 A123456（可能模型表述不同）")
    else:
...
async def test_persistence_and_recovery():
    ...
    await agent.chat("我的订单号是 B999888，想退货")
    await agent.chat("商品有质量问题，收到就是坏的")

    # 文件应该存在（SessionStore 保存到 {user_id}.json）
    session_file = Path(TEST_SESSION_FILE)
    if not session_file.exists():
        _fail(f"session 文件未生成: {session_file}")
    _ok("session 文件已落盘")

    raw = json.loads(session_file.read_text(encoding="utf-8"))
    if "messages" not in raw or "summary" not in raw:
        _fail(
            f"SessionStore 格式缺少 messages/summary 字段，实际 keys: {list(raw.keys())}"
        )
    if len(raw["messages"]) < 4:
        _fail(f"messages 条数预期至少 4，实际 {len(raw['messages'])}")
    _ok(f"SessionStore 格式正确（messages={len(raw['messages'])}）")

    # 模拟重启：新 Agent 实例加载同一文件
    agent2 = _fresh_agent()
    if agent2.history_size < 4:
        _fail(f"恢复的历史条数不对，预期至少 4，实际 {agent2.history_size}")
    _ok(f"新实例恢复了 {agent2.history_size} 条历史")

    # 追问上下文，验证模型能接上
    resp = await agent2.chat("我刚刚说的订单号是多少？")
...
async def test_reset():
    ...
    _clean()
    agent = _fresh_agent()
    await agent.chat("随便说一句话")
    if not Path(TEST_SESSION_FILE).exists():
        _fail("chat 后文件未生成")
...
# ---------- 测试 4：损坏 JSON 降级 ----------
@pytest.mark.asyncio
async def test_corrupted_json():
    print("\n[4/4] 损坏 JSON 降级测试")
    _clean()
    Path(TEST_SESSION_FILE).parent.mkdir(parents=True, exist_ok=True)
    Path(TEST_SESSION_FILE).write_text("{ this is not valid json", encoding="utf-8")
