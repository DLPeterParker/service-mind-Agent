...
import argparse  # 处理用户在终端敲代码时输入的参数
import sys  # 和 Python 解释器（运行代码的系统环境）交互
...
def main():
    ...
    args = parser.parse_args()  # 解析用户在终端敲的命令参数

    kb_dir = ROOT / settings.kb_dir  # 知识库目录
    ...
    by_doc: dict[str, int] = {}
    for c in chunks:
        by_doc[c.doc] = by_doc.get(c.doc, 0) + 1  # 统计每个文档的 chunk 数量
    ...
    # Up(date) + (In)sert 的组合词,如果数据库里没有这段文字，就插入；如果已经有了，就更新它。
    backend.upsert(
        chunks=chunks,
        vectors=vectors,
