...
def judge_task_completion(
    ...
    """任务完成度 judge：评估是否完成了期望任务列表中的任务。

    返回 (score 1-5, reason)。解析失败返回 (0.0, 原因)。
    expected_tasks 为 None 或空列表时，返回 (None, "")（不评分）。
    """
    if not expected_tasks:
        return None, ""
    tasks_text = "\n".join(f"- {t}" for t in expected_tasks)
    prompt = TASK_COMPLETION_PROMPT.format(
        user_input=user_input, reply=reply, expected_tasks=tasks_text
    )
    try:
        response = client.chat.completions.create(
            model=model,
