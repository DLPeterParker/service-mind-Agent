...
class IntentType(str, Enum):  # 继承自str和Enum，用于定义用户意图类型
    """用户意图类型"""
...
class CustomerServiceResponse(BaseModel):
    ...
    confidence: float = Field(
        description="意图识别的置信度，0.0-1.0",
        ge=0.0,
        le=1.0,  # 置信度必须大于等于0.0，小于等于1.0
    )
    reply: str = Field(description="给用户的回复内容")
    requires_human: bool = Field(description="是否需要转接人工客服", default=False)
